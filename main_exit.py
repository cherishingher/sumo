...
if agent_args.on_policy == True:
    ...
    for n_epi in range(args.epochs):
        ...
        for t in range(agent_args.traj_length):
            ...
            reward, R_comf, R_eff, R_safe ,R_exit= reward_info
            ...
            if done:
                ...
                if args.tensorboard:
                    ...
                    writer.add_scalar("score/exit", score_exit, n_epi)
    ...
    for n_epi in range(args.epochs):
        ...
        for t in range(agent_args.traj_length):
            if args.render:    
                env.render()
            # print('state shape',torch.from_numpy(state).size())
            ...
            if done:
                ...
                reward, R_comf, R_eff, R_safe, R_exit = reward_info
                next_state = np.clip((next_state_ - state_rms.mean) / (state_rms.var ** 0.5 + 1e-8), -5, 5)
                state=np.mean(state, axis=0).reshape(1, -1) 
                next_state=np.mean(next_state, axis=0).reshape(1, -1) 
                transition = make_transition(state,\
                                            action,\
                                            np.array([reward*args.reward_scaling]),\
                                            next_state,\
